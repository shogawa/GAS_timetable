...
v15では、古い端末向けの `legacy.html` にレスポンシブ表示、タップによる表示モード切替、焼き付き防止、最終更新時刻の詳細表示、明日の予定表示、ホーム画面追加用メタタグ、ファビコン・ホーム画面アイコン設定を追加しています。`Code.gs` と `index.html` は必ずセットで貼り替えてください。古いiPadでも表示したい場合は、`legacy.html` も追加してください。
