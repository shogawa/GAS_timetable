...
時計、現在の時限、残り時間、次の授業、Google Calendar予定、今日・明日の時間割、特別日課、曜日振替、個別変更、前期・後期切替、夏季・春季休業、端末別表示設定、夜間モード、全画面表示、PWA風ホーム画面追加、ファビコン・ホーム画面アイコンに対応しています。
...
v16では、URLパラメータ `device` による端末別設定に対応しています。`Code.gs` と `index.html` は必ずセットで貼り替えてください。古いiPadでも表示したい場合は、`legacy.html` も追加してください。
...
```text
https://script.google.com/macros/s/xxxxxxxxxxxx/exec?k=SECRET_KEY&device=phone
https://script.google.com/macros/s/xxxxxxxxxxxx/exec?k=SECRET_KEY&device=tablet
https://script.google.com/macros/s/xxxxxxxxxxxx/exec?k=SECRET_KEY&device=home
```

レガシー版と端末別設定を組み合わせる場合：

```text
https://script.google.com/macros/s/xxxxxxxxxxxx/exec?k=SECRET_KEY&legacy=1&device=tablet
```
...
| `device_setting` | 端末別の表示設定。`setting` の値を端末ごとに上書き |
...
| legacyTextSize | normal | レガシー表示の文字サイズ。`small` / `normal` / `large` / `xlarge` |
...
`setting` シートは全端末共通の基本設定です。`device_setting` シートに同じkeyを書くと、URLの `device` パラメータに一致する端末だけ、その値で上書きされます。

| device | key | value | memo |
|---|---|---|---|
| phone | defaultMode | normal | 机上スマホは通常表示 |
| phone | showSeconds | TRUE | スマホは秒あり |
| phone | refreshMinutes | 5 | 5分ごとに更新 |
| tablet | defaultMode | today | タブレットは今日詳細で起動 |
| tablet | showSeconds | FALSE | タブレットは秒なし |
| tablet | maxTomorrowPeriods | 8 | 明日の時間割を多めに表示 |
| home | defaultMode | tomorrow | 家では明日詳細で起動 |
| home | refreshMinutes | 15 | 家用はゆっくり更新 |
| tablet | legacyTextSize | large | レガシー版タブレットの文字を大きめにする |
| tablet | deviceLabel | 研究室タブレット | 右下に表示する端末名 |

### 処理順

設定は次の順で読み込まれます。後の設定ほど優先されます。
...
setting シート
↓
device_setting シートの device 一致行
```

### URL例

```text
?k=SECRET_KEY&device=phone
?k=SECRET_KEY&device=tablet
?k=SECRET_KEY&device=home
?k=SECRET_KEY&legacy=1&device=tablet
```

`device` 名は自由ですが、半角英数字、ハイフン、アンダースコアだけを使うのがおすすめです。
...
URLに `device` を付けると、その端末だけ `device_setting` シートの設定が反映されます。
...
右下には、端末名が設定されている場合に `端末 tablet` のように表示されます。`device_setting` で `deviceLabel` を指定すると、`端末 研究室タブレット` のような表示名にできます。
...
| 表示 | フル機能 | 基本表示中心 |
| 表示モード切替 | 対応 | タップで `通常` / `今日詳細` / `明日詳細` を切替 |
| レスポンシブ表示 | 対応 | 旧Safari向けの単純なレスポンシブに対応 |
| 夜間モード | 対応 | 自動・手動ON/OFFの簡易夜間表示 |
| 焼き付き防止 | 対応 | 数pxの位置ずらしに対応 |
| 明日の予定 | 対応 | `tomorrow.events` が取得できる場合に表示 |
| 文字サイズ調整 | 対応 | `legacyTextSize` で調整 |
| 画面ON | 対応ブラウザで利用可能 | 非対応 |
| 全画面 | 対応ブラウザで利用可能 | 非対応 |
| PWA風表示 | 対応 | ホーム画面追加用メタタグ・manifestリンクのみ対応 |
| ファビコン・ホーム画面アイコン | 対応 | `faviconUrl` / `appleTouchIconUrl` を利用 |
...
古いiPadで文字が小さい、または大きすぎる場合は、`setting` シートに `legacyTextSize` を追加します。

| key | value |
|---|---|
| legacyTextSize | `small` / `normal` / `large` / `xlarge` |

通常は `normal`、古いiPadを遠くから見る場合は `large` か `xlarge` がおすすめです。

### レガシー表示をホーム画面に追加したい

レガシー表示のURLをSafariで開き、共有ボタンから `ホーム画面に追加` を選びます。

```text
https://script.google.com/macros/s/xxxxxxxxxxxx/exec?k=SECRET_KEY&legacy=1
```

`setting` の `appleTouchIconUrl` を設定している場合、レガシー表示でもホーム画面アイコンとして使われます。`faviconUrl` が設定されている場合は、ブラウザのファビコン用リンクも出力されます。
...
- URLパラメータ `device` による端末別設定に対応
- `device_setting` シートを追加
- `setting` の共通設定を、端末ごとに上書きできるように変更
- 通常版・レガシー版の両方で `device` をGAS側に渡すように変更
- 右下の表示に端末名を表示
- manifest の `start_url` に `device` と `legacy` を反映

### v15

- レガシー表示にレスポンシブ対応を追加
- レガシー表示にタップで `通常` / `今日詳細` / `明日詳細` を切り替える機能を追加
- レガシー表示に焼き付き防止の位置ずらしを追加
- レガシー表示に最終更新時刻の詳細表示を追加
- レガシー表示に明日のGoogle Calendar予定表示を追加
- レガシー表示に `legacyTextSize` による文字サイズ調整を追加
- レガシー表示にホーム画面追加用メタタグ、manifestリンク、ファビコン、Apple Touch Iconリンクを追加
